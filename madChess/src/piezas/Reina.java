...
//    	        		(movimiento[0] == 1 && (movimiento[1] == -1 || movimiento[1] == 1 || movimiento[1] == 0 )  )
//    	        		;
//
        ...
        for (int[] movimiento : movimientos) { //Recorre cada movimiento posible desde la posicion de la torre
            ...
            while (nuevaFila >= 0 && nuevaFila <= 7 && nuevaColumna >= 'A' && nuevaColumna <= 'H') {
                ...
                if ((casillaDisp.getPieza()!=null&&casillaDisp.getPieza().getIsWhite()==this.getIsWhite())||
                		(contador == 1 && casillaDisp.getPieza()== null)) {break;}
                casillasDisp.add(casillaDisp);

                // Avanzo a la dirección del movimiento
                nuevaFila += movimiento[0];
                nuevaColumna = (char) (nuevaColumna + movimiento[1]);
                if (casillaDisp.getPieza()!=null && isAlter && casillaDisp.getPieza().getIsWhite() != this.getIsWhite() ) {
                	contador++;
                	
                }
                
                if (	(isAlter && (contador==2)) ||
                		(!isAlter&&casillaDisp.getPieza()!=null)
                		) {break;}
